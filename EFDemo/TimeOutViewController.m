...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        self.tabBarItem.image = [UIImage imageNamed:@"timeOut@2x"];
...
- (void)viewDidLoad {
    ...
    _adViewTime = [[EFAdView alloc] initWithAdUnitSize:CGSizeMake(320, 568) andLocation:CGPointMake(0, 0)];
    _adViewTime.EFDelegate = (id)self;
    _EFTime = [[EFTimeOutController alloc] initWithViewController:self andAdView:_adViewTime andDelegate:self];
...
- (void)loadAdWithAdID {
    NSString *adID = [_EFTime updateAdID:_text.text];
    
    if ([_EFTime loadTimeOut]) {

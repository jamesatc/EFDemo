...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        self.tabBarItem.image = [UIImage imageNamed:@"banner@2x"];
...
- (void)viewDidLoad {
    ...
    _EFBanner = [[EFBannerController alloc] initWithViewController:self andAdView:_adView andDelegate:self];
...
- (void)showAdWithID {
    ...
    if ([adID integerValue]) {
        _label.text = [NSString stringWithFormat:@"Updated to adID: %@", [_EFBanner updateAdID:adID]];
    } else {
        _label.text = [NSString stringWithFormat:@"Updated to adID: %@", [_EFBanner getAdID]];
    }
